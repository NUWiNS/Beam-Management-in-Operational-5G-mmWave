...
def reorganize_data_drive(data):
    ...
    s21_ul.extend(data['s21']['Uplink']['Drive'])

    # Process s24 data 
    s24_dl.extend(data['s24']['Downlink']['Drive'])
    s24_ul.extend(data['s24']['Uplink']['Drive'])
...
def data_to_box(s21_walk, s21_drive, s24_walk, s24_drive, save_flag, show_flag):
    ...
    # Print statistics
    print(f'Tput Statistics (Downlink):')
    ...
    ax.text((positions[0] + positions[1])/2, ylim[1] + 13, 'S21', 
            horizontalalignment='center', verticalalignment='bottom', fontsize=25)
    ax.text((positions[2] + positions[3])/2, ylim[1] + 13, 'S24',
            horizontalalignment='center', verticalalignment='bottom', fontsize=25)
...
s21_walk_dl, _, s24_walk_dl, _ = reorganize_data_walk(loaded_walk_data)
s21_drive_dl, _, s24_drive_dl, _ = reorganize_data_drive(loaded_drive_data)
